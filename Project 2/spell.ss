...
(define hashw
  (lambda (hashlist word)
    (if (not (null? hashlist))
        (cons ((car hashlist) word)(hashw (cdr hashlist) word))
        '())
    ))
...
(define hashd
  (lambda (hashlist dict)
    (if (not (null? dict))
        (if (not (null? hashlist))
            (cons (hashw hashlist (car dict)) (hashd hashlist (cdr dict)))
            '())
        '())
    ))
...
(define compare
  (lambda (dictw word)
    (if (null? word)
        '()
        (if (= (car word) (car dictw))
            (cons #t (compare (cdr word) (cdr dictw)))
            (cons #f (compare (cdr word) (cdr dictw))))
        )
    ))
...
(define gen-checker
  ...
  (lambda (word)
    (comparew (hashd hashfunctionlist dict) (hashw hashfunctionlist word))
)))
